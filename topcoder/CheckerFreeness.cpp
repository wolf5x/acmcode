...
struct point {
	...
	friend long long xmul(point a, point b) {
		return a.x*b.y - a.y*b.x;
	}

	friend long long dmul(point a, point b) {
		return a.x*b.x + a.y*b.y;
	}

	friend double angle(point a, point b) {
		if(dmul(a,a) == 0 || dmul(b,b) == 0) 
			return 0;
		return acos(dmul(a, b)/sqrt(dmul(a,a)*dmul(b,b)));
	}
};

int nw, nb;
vector<long long> wx, wy, bx, by;
vector<point> wp, bp;
...
void readpoint(vector<long long> &vx, vector<long long> &vy, vector<point> &vp) {
	vp.clear();
	REP(i, vx.size())
