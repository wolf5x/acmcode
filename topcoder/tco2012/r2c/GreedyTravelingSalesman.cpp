...
class GreedyTravelingSalesman {
    public:
    int worstDistance(vector <string> f1, vector <string> f2, vector <string>f3, vector <string> f4) {
        n = f1.size();
        memset(w, 0, sizeof(w));
        REP(i,n) REP(j,n){
            e[i][j].clear();
            w[i][j] = (f1[i][j]-'0')*1000 + (f2[i][j]-'0')*100 + (f3[i][j]-'0')*10 + (f4[i][j]-'0');
        }
        ...
        REP(i,n) REP(j,n) if(i!=j) REP(k, e[i][j].size()){
            int t = w[i][j];
            if(e[i][j][k] > 1){
                w[i][j] = min(9999, e[i][j][k]-1);
                ans = max(ans, solve());
            }
            w[i][j] = min(9999, e[i][j][k]);
            ans = max(ans, solve());
            w[i][j] = t;
